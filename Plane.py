...
class Plane:
    def __init__(self, name, model, capacity, x=0, y=0, altitude=0, heading=0, speed=0.2,
                  max_speed_change=5, max_heading_change=3, max_altitude_change=100, id=0):
        self.name = name
        self.model = model
    ...
    @property
    def dp(self):
        return self.airport.position - self.position

    @property
    def distance_to_airport(self):
        return np.linalg.norm(self.dp)

    @property
    def angle_to_airport(self):
        return angle_between_points(self.dp)

    @property
    def heading_error(self):
        return (self.angle_to_airport - (self.heading % 360) + 180) % 360 - 180 
