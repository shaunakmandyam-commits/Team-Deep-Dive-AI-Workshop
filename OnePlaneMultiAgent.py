...
class OnePlaneMultiAgent(MultiAgentEnv):
    ...
    def __init__(self, config=None, dt=1, render_mode=None, speed=1, difficulty = 1):
        super().__init__()
        self.dt = dt
        self.steps = 0
    ...
    def _plane_observation(self, index):
        ...
        # 0: dx, 1: dy, 2: speed, 3: sin heading, 4: cos heading,
        # 5: bearing error, 6: distance to airport, 7: time left
    ...
    def _obs(self):
        ...
        for agent in self.agents:
            observations[agent] = self._plane_observation(self.sim.name_to_index[agent])
        return observations
    ...
    def _reset_agent(self, plane_angle, agent):
        ...
        plane_distance = self.np_random.uniform(small - 1, small) / (6.1 - self.difficulty)
        airport_distance = self.np_random.uniform(small - 1, small) / (6.1 - self.difficulty)
        ...
        airport_angle += np.clip(self.np_random.normal(0, self.difficulty * 30), -180, 180)

        return (Plane(agent, "plane_policy", 100, plane_pos[0], plane_pos[1], heading=airport_angle % 360),
                Airport(airport_pos[0], airport_pos[1]))

    def _random_reset(self, agent):
        width, height = self.sim.to_scale()
        

        plane_angle = self.np_random.uniform(0, 360)

        plane_pos = np.array([self.np_random.uniform(0, width), self.np_random.uniform(0, height)])
        airport_pos = np.array([self.np_random.uniform(0, width), self.np_random.uniform(0, height)])

        return (Plane(agent, "plane_policy", 100, plane_pos[0], plane_pos[1], heading=plane_angle),
                Airport(airport_pos[0], airport_pos[1]))
    ...
    def reset(self, *, seed = None, options = None):
        ...
        for agent in self.agents:
            ...
            plane, airport = self._random_reset(agent)
